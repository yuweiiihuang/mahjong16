...
def test_deadwall_fixed_flow():
    ...
    env = Mahjong16Env(Ruleset(include_flowers=False, dead_wall_mode="fixed", dead_wall_base=16), seed=1)
    env.reset()
...
def test_deadwall_gang_plus_one():
    ...
    env = Mahjong16Env(Ruleset(include_flowers=False, dead_wall_mode="gang_plus_one", dead_wall_base=16), seed=2)
    env.reset()
